...
from tkinter import Tk,Button,Label,messagebox,simpledialog,Scrollbar,RIGHT,Y,YView,Menu,OptionMenu,ttk,PhotoImage,filedialog,scrolledtext
...
def help():
    ...
    ht.pack()
    v = Label(win,text="版本4.0",font=('微软雅黑 Light',20),bg="#66ccff")
    v.pack()
...
def pip_install_file():
    install_path = filedialog.askopenfilename(title = "选择whl文件",filetypes = (("WHL模块文件","*.whl"),("zip模块文件","*.zip"),("tar.gz模块文件","*.tar.gz")))
    if install_path == '':
        return
...
def pip_install():
    ...
    title.pack()
    file = Button(iw,text="安装本地模块文件",font=("微软雅黑 Light",25),command=pip_install_file,width=17,height=1,bg='#ffffff',bd=0)
...
def pip_uninstall():
    ...
    if (unpy == None) or (unpy == ''):
        return
    os.system('start pip uninstall ' + unpy)
    messagebox.showinfo('提示','卸载完成')
def pip_list():
...
def download_package():
    dpy = simpledialog.askstring('提示','请输入模块名：')
    if (dpy == None) or (dpy == ''):
        return 
    a = messagebox.askyesnocancel('提示','请问需要输入版本号吗？')
    if a:
        all = '请输入' + dpy + '要下载的版本号：'
        dpyint = simpledialog.askstring('提示',all)
        if (dpyint == None) or (dpyint == ''):
            return
        os.system('start pip download ' + dpy + '==' + dpyint)
        messagebox.showinfo('提示','安装完成')
    if not a:
        os.system('start pip download ' + dpy)
        messagebox.showinfo('提示','安装完成')
    if a == None:
        return
...
def pip_shell():
    ...
    scr1 = scrolledtext.ScrolledText(ps,width=50,height=3,font=('Microsoft YaHei UI',10),bd=0)
    scr1.pack()
    def start_pip_shell_command():
        
        scrout = scr1.get(1.0,'end').rstrip('\n')
        

        scr1.delete(1.0,'end')
        out = os.popen(scrout).read()
        scr2.delete(1.0,'end')
        
        scr2.insert('end',out)
        
    s = ttk.Button(ps,text='运行(换行会自动拼接成一条命令[Beta])',command=start_pip_shell_command,width=50)
    s.pack()
    scr2 = scrolledtext.ScrolledText(ps,width=50,height=10,font=('Microsoft YaHei UI',10),bd=0)
    scr2.pack()
...
root.deiconify()
root.title('pipGUI')
root.geometry('626x583+451+95')
root.config(bg='#66ccff')


title = Label(text="pipGUI 主页",font=("微软雅黑 Light",23),bg='#66ccff')
title.pack()

b1 = Button(text="安装模块",font=("微软雅黑 Light",17),command=pip_install,width=17,height=1,bg='#ffffff',bd=0)

b1.pack()

b2 = Button(text="查看已安装的模块",font=("微软雅黑 Light",17),command=pip_list,width=17,height=1,bg='#ffffff',bd=0)

b2.pack()

b3 = Button(text="卸载模块",font=("微软雅黑 Light",17),command=pip_uninstall,width=17,height=1,bg='#ffffff',bd=0)

b3.pack()

b4 = Button(text="升级pip",font=("微软雅黑 Light",17),command=pipu,width=17,height=1,bg='#ffffff',bd=0)
b4.pack()

b5 = Button(text="升级指定包",font=("微软雅黑 Light",17),command=update_package,width=17,height=1,bg='#ffffff',bd=0)
b5.pack()
b6 = Button(text="查看pip版本",font=("微软雅黑 Light",17),command=get_pip_ver,width=17,height=1,bg='#ffffff',bd=0)
b6.pack()
b7 = Button(text="查看需更新的包",font=("微软雅黑 Light",17),command=cheak_package_update,width=17,height=1,bg='#ffffff',bd=0)
b7.pack()
b8 = Button(text="卸载pip(慎用)",font=("微软雅黑 Light",17),command=uninstall_pip,width=17,height=1,bg='#ffffff',bd=0)
b8.pack()
b9 = Button(text="下载whl文件",font=("微软雅黑 Light",17),command=download_package,width=17,height=1,bg='#ffffff',bd=0)
b9.pack()
b10 = Button(text="启动pip Shell",font=("微软雅黑 Light",17),command=pip_shell,width=17,height=1,bg='#ffffff',bd=0)
b10.pack()
root.mainloop()
